...
##
## Revision:
##  v1.0.0 - ruqin - 07/26/2018 - Build the script.
#######################################################################################
...
LogPrint "DEBUG: Server_Adapter: ${Server_Adapter}."
...
LogPrint "INFO: iperf3 -s has been executed in Guest-A."
...
LogPrint "INFO: Network connection works before iperf test."
...
LogPrint "INFO: iperf3 test has beeing started."
...
if ($null -eq $LRO_count) {
    LogPrint "ERROR: Current LRO <pkts rx> should not be null!"
    Stop-VM $testVM -Confirm:$False -RunAsync:$true
    DisconnectWithVIServer
    return $Aborted
}
...
LogPrint "INFO: LRO has been enabled."
...
LogPrint "INFO: LRO has been disabled"
...
Stop-VM $testVM -Confirm:$False -RunAsync:$true
if (-not $?) {
    LogPrint "ERROR: Cannot stop VM ${testVMName}."
    DisconnectWithVIServer
    return $Aborted
}

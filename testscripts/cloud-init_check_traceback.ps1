########################################################################################
## Description:
##  Check no "traceback" level message in /var/log/cloud-init.log
##
## Revision:
...
.Description
	<test>
            <testName>cloud-init_check_traceback</testName>
            <testID>ESX-cloud-init-012</testID>
            <setupScript>setupscripts\add_vmxnet3.ps1</setupScript>
            <testScript>testscripts/cloud-init_check_traceback.ps1</testScript  >
            <files>remote-scripts/utils.sh</files>
            <testParams>
                <param>nicName=auto-test</param>
                <param>TC_COVERED=RHEL6-0000,RHEL-187965</param>
            </testParams>
...
#  Check no "traceback" level message in /var/log/cloud-init.log
$loginfo = bin\plink.exe -i ssh\${sshKey} root@${ipv4Addr_clone} "grep 'traceback' /var/log/cloud-init.log"
if ($null -eq $loginfo)
{
    $retVal = $Passed

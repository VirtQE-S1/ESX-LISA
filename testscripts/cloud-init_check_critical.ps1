...
.Description
	<test>
            <testName>cloud-init_check_critical</testName>
            <testID>ESX-cloud-init-009</testID>
            <setupScript>setupscripts\add_vmxnet3.ps1</setupScript>
            <testScript>testscripts/cloud-init_check_critical.ps1</testScript  >
            <files>remote-scripts/utils.sh</files>
            <testParams>
                <param>nicName=auto-test</param>
                <param>TC_COVERED=RHEL6-0000,RHEL-187892</param>
            </testParams>
...
{
    $retVal = $Passed
    LogPrint "INFO: The customization guest passed with no critical log ${loginfo}."
    
}
else
{
    LogPrint "ERROR: The customization guest failed with critical log ${loginfo}."
    RemoveVM -vmName $cloneName -hvServer $hvServer
    return $Failed
}

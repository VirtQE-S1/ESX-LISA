########################################################################################
## Description:
##  Check no "ERROR" level message in /var/log/cloud-init.log
##
## Revision:
...
.Description
	<test>
            <testName>cloud-init_check_error</testName>
            <testID>ESX-cloud-init-008</testID>
            <setupScript>setupscripts\add_vmxnet3.ps1</setupScript>
            <testScript>testscripts/cloud-init_check_error.ps1</testScript  >
            <files>remote-scripts/utils.sh</files>
            <testParams>
                <param>nicName=auto-test</param>
                <param>TC_COVERED=RHEL6-0000,RHEL-187893</param>
            </testParams>
...
#  Check no "ERROR" level message in /var/log/cloud-init.log
$loginfo = bin\plink.exe -i ssh\${sshKey} root@${ipv4Addr_clone} "grep 'ERROR' /var/log/cloud-init.log"
if ($null -eq $loginfo)
{
    $retVal = $Passed
    LogPrint "INFO: The customization guest passed with no error log ${loginfo}."
    
}
else
{
    LogPrint "ERROR: The customization guest failed with error log ${loginfo}."
    RemoveVM -vmName $cloneName -hvServer $hvServer
    return $Failed
}
